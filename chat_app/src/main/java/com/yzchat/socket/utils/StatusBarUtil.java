...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (fakeStatusBarView != null) {
                if (fakeStatusBarView.getVisibility() == GONE) {
     ...
     * 设置透明并全屏
     */
    public static void setTransparentForWindowAndDarkIcon(Activity activity, int flagss) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().setStatusBarColor(Color.TRANSPARENT);
            int systemUiVisibility = View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                systemUiVisibility = systemUiVisibility | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
                setMIUIStatusBarDarkIcon(activity, true);
                setMeizuStatusBarDarkIcon(activity, true);
    ...
    public static void hideFakeStatusBarView(Activity activity) {
        ...
        View fakeStatusBarView = decorView.findViewById(FAKE_STATUS_BAR_VIEW_ID);
        if (fakeStatusBarView != null) {
            fakeStatusBarView.setVisibility(GONE);
        }
        View fakeTranslucentView = decorView.findViewById(FAKE_TRANSLUCENT_VIEW_ID);
        if (fakeTranslucentView != null) {
            fakeTranslucentView.setVisibility(GONE);
        }
    }
     ...
     * 全屏隐藏状态栏
     */
    private static int flagss = 0;

    public static void setFullScreen(Activity activity) {
        Window window = activity.getWindow();
        if (flagss == 0)
            flagss = window.getDecorView().getSystemUiVisibility();
    ...
    public static void showBottomUIMenu(Activity context) {
        if (context == null) return;
        Window window = context.getWindow();
        //显示虚拟按键

        if (Build.VERSION.SDK_INT >= 19 && flagss != 0) {
            View decorView = window.getDecorView();
//            int uiOptions = View.SYSTEM_UI_FLAG_VISIBLE;
            decorView.setSystemUiVisibility(flagss);
    ...
    @TargetApi(Build.VERSION_CODES.M)
    public static void setLightModeOnly(Activity activity) {
        setMIUIStatusBarDarkIcon(activity, true);
        setMeizuStatusBarDarkIcon(activity, true);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
...
//            activity.getWindow().setStatusBarColor(ContextCompat.getColor(activity, R.color.white));
    ...
    private static void addTranslucentView(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        ...
        if (fakeTranslucentView != null) {
            if (fakeTranslucentView.getVisibility() == GONE) {
     ...
     * 移除黑色字体
     */
    public static void removeBlackFont(Activity activity) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Window window = activity.getWindow();
...
//            NotchFit.fit(activity, NotchScreenType.FULL_SCREEN, null);
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//    }
